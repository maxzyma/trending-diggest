...
This weighted sum of many n-step returns is called λ-return $G_t^{\lambda} = (1-\lambda) \sum_{n=1}^{\infty} \lambda^{n-1} G_t^{(n)}$. TD learning that adopts λ-return for value updating is labeled as **TD(λ)**. The original version we introduced [above](https://lilianweng.github.io/posts/2018-02-19-rl-overview/#value-estimation) is equivalent to **TD(0)**.
