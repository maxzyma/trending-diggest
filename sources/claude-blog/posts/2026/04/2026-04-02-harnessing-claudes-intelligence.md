...
- 喇叭芽的催眠+紧束组合：在催眠粉命中之前用咬住快速击倒它。别让它布场！
- 第一世代背包上限：最多 20 个道具。进地下城前丢掉不需要的招式机。
- 旋转地砖迷宫：不同的进入 y 位置会通向不同的目的地。尝试所有入口，并串联穿过多个区块。
- B1F y=16 的墙在所有 x=9-28 处已确认为实心（第 14557 步）
```
