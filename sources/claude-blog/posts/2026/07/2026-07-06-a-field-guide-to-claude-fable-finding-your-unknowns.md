...
title_zh: Claude Fable 5 实战指南：发现你的未知
...
- 作者用「地图与疆域」的比喻区分你给 Claude 的提示、技能与上下文，以及工作真正发生的代码库与现实约束，两者的差距即所谓的「未知」。
- Fable 5 是第一个让作者感到工作质量瓶颈在于「自己能否澄清未知」而非模型能力的模型。
- 将问题拆为四类：已知的已知、已知的未知、未知的已知、未知的未知；优秀的智能体编程者未知更少，也更懂得为未知预留空间。
- 指令过于具体会让 Claude 死板执行而错失更优路径，过于模糊则会让它套用未必贴合的行业惯例；关键在于给足起点上下文，把 Claude 当作思考伙伴。
- 实施前可用「盲点扫描」发现未知的未知、用头脑风暴与原型提前暴露未知的已知、用访谈与参考资料对齐意图，并产出实施计划。
- 实施中通过实施笔记记录，实施后用推介稿、讲解稿与小测验来验证与巩固理解。
- 发现未知是贯穿实施前、中、后的迭代过程，有时未知会指向应当换一种方式解决问题。

## 正文

在使用 Claude Code 时，我常常想起「地图」与「疆域」之间的区别。

> When working with Claude Code, I’m often reminded of the difference between the map and the territory.

地图，是对待完成工作的一种表示，它就是我的提示词、技能和上下文，是我交给 Claude 的东西。疆域则是工作真正需要发生的地方，是代码库、真实世界及其实际约束。
...
地图与疆域之间的差异，就是我所说的未知项（unknowns）。当 Claude 遇到一个未知项时，它需要基于对我意图的最佳猜测来做决定。要做的工作越多，Claude 可能遇到的未知项就越多。
...
Claude Fable 是第一个让我觉得工作质量的瓶颈在于我澄清其未知项能力的模型。
...
我发现，与 Fable 协作是一个在实现之前、之中和之后不断发现自己未知项的迭代过程。
...
你的未知是什么？当我带着一个问题来找 Claude 时，我倾向于将其分解为四类：

> What are your unknowns? When I come to Claude with a problem I tend to break it down in 4 ways:

- 已知的已知：这本质上就是我提示词里的内容。我告诉智能体（agent）我想要什么？
- 已知的未知：有哪些我还没想清楚，但我意识到自己还没想清楚的？
- 未知的已知：有哪些东西太显而易见，以至于我永远不会写下来，但一看到就会认出来？
- 未知的未知：有哪些是我压根没考虑过的？有哪些知识是我根本没意识到的？我知道某件事能做到多好吗？
...
最好的智能体编程者（agentic coder）相对而言未知很少。看着像 Boris 或 Jarred 这样的人写提示词，我很明显能看出他们详细地知道自己想要什么。他们与代码库和模型行为都深度同步。
...
但他们同样会预设未知的存在。在很多方面，减少并为你的未知做规划正是智能体编程的核心技能。而幸运的是，这是一项你可以通过与 Claude 协作而不断精进的技能。
...
给 Claude 下达指令是一种微妙的平衡。如果你过于具体，Claude 会照做你的指令，即便调整方向可能更合适。如果你过于含糊，Claude 往往会基于行业最佳实践做出选择和假设，而这些未必契合你的任务。
...
当你没有考虑到自己的未知项时，两个方向都会失败。你不知道路上何时会布满障碍，也不知道路上何时会畅通无阻，但你仍然希望 Claude 转向。
...
Claude 能帮你更快地发现自己的未知项。它可以极快地搜索你的代码库和互联网，而且它对一般话题的了解远超你。它也能更快地从失败中迭代。
...
这个过程中最重要的一环，是给 Claude 提供关于你起点的背景信息。比如，告诉它你的思考进行到了哪一步；坦白你对该问题和代码库的经验；让它像一个思维伙伴那样与你协作。
...
> • Quizzes
### 实现前\nb036 盲点排查\nb037 p 刚开始工作时，最有用的事情之一就是了解自己的盲点。例如，如果你要在代码库中一个陌生的部分编写功能，或者用 Claude 帮你完成不熟悉的工作（比如迭代某个设计），你很可能会遇到大量的未知的未知（unknown unknowns）。
...
开始工作时，最有用的事情之一就是了解自己的盲点。例如，如果你正在代码库的一个新区域编写功能，或者使用 Claude 帮你完成不熟悉的工作（比如迭代设计），你很可能存在大量的未知的未知 。
...
你可能不知道该问什么问题、什么样才算好、以前做过哪些工作、又该避开哪些陷阱。

> You may not know what questions to ask, what good looks like, what historical work has been done, or what potholes to avoid.

在这些情况下，你可以让 Claude 帮你找出这些未知的未知并向你解释。我喜欢直接使用“盲点排查（blind spot pass）”和“未知的未知”这样的字眼。向它提供关于你是谁、你知道什么的背景信息，通常对 Claude 理解如何与你更好地协作很重要。
...
- “我正在添加一个新的身份认证提供方（auth provider），但我对这个代码库里的认证模块一无所知。你能做一次盲点排查，帮我找出相关的未知的未知，并帮我更好地向你提问吗。”
- “我不知道什么是调色（color grading），但我需要给这段视频调色。你能教我理解自己在调色方面的未知的未知，好让我能更好地提问吗？”
...
当我在一个存在大量未知的已知（unknown knowns）的领域工作时，也就是那些我只有看到才知道如何定义的标准，我喜欢让 Claude 和我一起做头脑风暴和原型。
...
在做原型时尽早识别并明确说出未知的已知极其有价值，因为在实现阶段才发现它们的代价（相对）更高。功能或规格上的微小改动可能导致代码实现天差地别，而且让你的智能体撤回此前的改动可能会更加困难。
...
例如，你可能只是想看看在一个框里加个按钮是什么样子，而不必接上后端路由或在前端维护额外的状态。
...
另一个例子是视觉设计，对我来说这是很难用语言表达、但一看到就知道自己想要什么的东西。这种情况下，我会为某个产物索要几种不同的设计方案。
...
我几乎每次编码会话都以探索或头脑风暴阶段开始。这有助于我带着意图去定义项目的范围。Claude 常常能找到我本会错过的高价值方案，但有时也会只见树木不见森林。头脑风暴能防止我把范围设定得过窄或过宽。
...
- “我想为这些数据做一个仪表盘，但我没有视觉品味，也不知道能做成什么样。给我做一个 HTML 页面，包含 4 个截然不同的设计方向，让我可以对它们做出反应。”
- “在接任何东西之前，用假数据做一个单独的 HTML 文件来模拟新的编辑器工具栏。我想先对布局做出反应，然后你再去动真正的应用。”
- “这是我大致的问题：用户在引导（onboarding）之后就流失了。搜索代码库，头脑风暴出 10 个我们可以干预的地方，从最省事的到最有雄心的都列出来。我会告诉你哪些能引起我的共鸣。”
...
等我做完足够的头脑风暴后，我很可能仍然存在一些未知。

> Once I’ve done sufficient brainstorming, I likely still have unknowns.

这种情况下，我会让 Claude 就任何未知或含糊之处对我进行访谈。让 Claude 访谈你时，尽量向它提供关于你问题的背景，以引导它的提问。
...
- “就任何含糊之处每次问我一个问题来访谈我，优先问那些我的回答会改变架构的问题。”
...
有时你无法详细描述自己想要什么。例如，你可能没有相应的表达语言，或者它太复杂，描述起来会花上你相当长的时间。
...
这种情况下，最好的办法是给出一个参考。虽然你可以附上图表、文档或图片，但绝对最好的参考是源代码（source code）。
...
如果你有一个以某种特定方式实现了某功能的库，或者一个你非常喜欢的设计组件，只需把 Fable 指向那个文件夹并告诉它要找什么即可，即使它是用另一种语言写的也没关系。相比于比如说一张截图，这能为 Claude 提供关于标记（markup）和结构的丰富得多的细节。
...
- “vendor/rate-limiter 里的这个 Rust crate 实现了我想要的那种确切的退避（backoff）行为。读一下它，并在我们的 TypeScript API 客户端里重新实现相同的语义。”
...
当我认为自己准备好实现时，我往往会让 Claude 整理一份实现计划供我审阅。计划聚焦于那些最可能发生变化的部分，比如数据模型、类型接口或用户体验流程。这让 Claude 能把我实际上可能需要修改的东西呈现出来。
...
- “用 HTML 写一份实现计划，但要把我最可能调整的决策放在前面：数据模型的改动、新的类型接口，以及任何面向用户的东西。把机械性的重构埋到最底部，那部分我信任你。”
...
一旦我对计划感到满意，我会新建一个会话，并把各类产物传入提示词。这会给 Claude 一个全新的上下文窗口，同时保留它在规划阶段整理出的全部信息。例如，我可能传入一份规格文件和一个原型，让智能体去实现它。
...
但事实是，无论你做多少规划，总有未知的未知（unknown unknowns）潜伏其中。智能体在工作过程中可能会发现代码里存在某个边缘情况，迫使它改用另一种方案。
...
我会让 Claude Code 维护一个临时的 'implementation-notes.md'（或 .html）文件，在其中记录它所做的决定，方便我们在下一次尝试时从中学习。
...
- “维护一个 implementation-notes.md 文件。如果你遇到某个边缘情况，迫使你偏离计划，就选择保守的方案，把它记录在 'Deviations'（偏离）一节下，然后继续推进。”
...
交付一个东西最重要的环节之一，是获得认同和批准。在最终文档中构建推介和讲解材料有助于：
...
- 当专家希望看到你已经考虑了他们本会预见的未知点和常见失败点时，加速批准
...
- “把原型、规格说明和实现笔记打包成一份文档，让我能丢进 Slack 去争取认同。开头放演示 GIF。”
...
经过一段长时间的工作后，Claude 完成的东西可能远比我意识到的要多。仅仅阅读代码差异只能让我对发生了什么有粗浅的理解，因为很多行为都取决于已有的代码路径。
...
在给我大量背景之后，让 Claude 就这次改动来测验我，有助于我理解到底发生了什么。只有在完美通过测验后，我才会合并。
...
- “我想确保自己完全理解这次改动中发生的一切。给我一份关于这些改动的 HTML 报告，让我带着背景、直觉去阅读和理解，说明做了什么等等，并在末尾附上一份我必须通过的关于这些改动的测验。”
...
所以我从自己已知的部分入手。我知道 Claude 可以用代码来剪辑视频并进行转录，但我不确定它是否足够准确。于是我请 Claude 向我解释像 Whisper 这样的转录是如何工作的，以及我能否用 ffmpeg 准确地剪掉诸如"呃"这样的语气词或较长的停顿。
...
我希望 Claude 创建一个与我所说的话同步的界面（UI），但我不确定这是否可行，所以我请 Claude 用 Remotion 和一份转录稿制作一个原型视频，看看是否行得通。
...
最后，视频本身看起来有点暗淡，我知道这是调色（color grading）的结果，但我并不真正了解调色是什么。我的第一次尝试是让 Claude 做几个不同的版本供我挑选，但我意识到，在调色方面我并不知道"好"应该是什么样子。于是我改而请 Claude 教我了解调色，以此发现我的未知之处。
...
模型越强，用对方法能做成的事就越多。当一个长周期任务返回了错误结果，往往说明你需要花更多时间界定未知项，或制定一份能让你和 Claude 在过程中不断调整的实施计划。
...
每一次讲解、头脑风暴、访谈、原型和参考，都是一种廉价的方式，让你在代价高昂之前就发现自己原本不知道的东西。
...
所以，不妨在开始下一个项目时，请 Claude 帮你找出你的未知项。
