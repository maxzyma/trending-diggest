...
- **落点依据**：规格来源 behaviors/site-skeleton.gherkin#SC-01/02 + contracts.md#构建/部署契约 / 代码落点 本仓根 `_config.yml`、`.github/workflows/pages.yml`、`CNAME`（新建） / 定位方式 ⚠️ 前置确认项：理由——本仓当前无 Jekyll、需新建这些文件（首个 Jekyll 化，无既有落点可追踪）；确认方式——implement 时参考 github-trending-digest `_config.yml`+`pages.yml` 结构（实证：那仓已跑通同栈）落地并确认
...
- **落点依据**：规格来源 behaviors/portal-homepage.gherkin#SC-04~07/21 + ui/prototype.html（视觉契约，implement 照它建）+ entities.md#SourceCard / 代码落点 `_layouts/portal-home.html`、`_data/sources.yml`、`index.md`（新建） / 定位方式 ⚠️ 前置确认项：理由——需新建页面（门户首页不存在，无既有落点可追踪）；确认方式——implement 照 `specs/features/aggregation-portal/ui/prototype.html` 建 layout 并确认视觉契合
...
- **落点依据**：规格来源 behaviors/legacy-redirect.gherkin#SC-17~19/25 + algorithms.md#ALG-02 + contracts.md#301 fixture 契约 / 代码落点 theuntold Worker（同 TASK-005） / 定位方式 ⚠️ 前置确认项：理由——跨仓待协调（Worker 代码归 theuntold 仓、需其仓当前态+部署凭据，本 worktree 无落点可追踪）；确认方式——implement 前跨仓确认，随 TASK-005 Worker 一并落地
