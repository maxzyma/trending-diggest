...
# SC-23b claude_blog collection 整体缺失 → 非零（codex 跨家族审补：缺失比前缀错位更严重）
cp _config.yml /tmp/vb_c.bak; ruby -e "require 'yaml';require 'date';c=YAML.load_file('_config.yml');c['collections'].delete('claude_blog');File.write('_config.yml',c.to_yaml)"; ruby scripts/validate-site.rb >/dev/null 2>&1; rc=$?; cp /tmp/vb_c.bak _config.yml; assert "SC-23b/collection-missing" "挂载 collection 缺失 fail-loud" "$( [ $rc -ne 0 ] && echo 0 || echo 1 )"
# SC-23c claude_blog 缺 output:true → 非零
cp _config.yml /tmp/vb_c.bak; ruby -e "require 'yaml';require 'date';c=YAML.load_file('_config.yml');c['collections']['claude_blog'].delete('output');File.write('_config.yml',c.to_yaml)"; ruby scripts/validate-site.rb >/dev/null 2>&1; rc=$?; cp /tmp/vb_c.bak _config.yml; assert "SC-23c/collection-no-output" "collection 缺 output:true fail-loud" "$( [ $rc -ne 0 ] && echo 0 || echo 1 )"
